...
public class ImportarFragment extends Fragment {
    ...
    private String tipoArchivo; // "csv" o "pdf"
    private List<Residuo> listaPreview;

    // Launcher para abrir selector de archivos CSV
    ...
    // Launcher para abrir selector de archivos PDF/texto
    ...
    private void mostrarArchivoSeleccionado() {
        ...
        // Pre-leer para mostrar cantidad de registros
        previewArchivo();
    }
        ...
        new Thread(() -> {
            try {
                ...
                if ("csv".equals(tipoArchivo)) {
                    listaPreview = FileManager.importarCSV(requireContext(), uriSeleccionado, opId, opNombre);
                } else {
                    listaPreview = FileManager.importarDesdeTexto(requireContext(), uriSeleccionado, opId, opNombre);
                }
                requireActivity().runOnUiThread(() -> {
                    progressImportar.setVisibility(View.GONE);
                    int n = listaPreview.size();
                    if (n > 0) {
                        tvResultado.setText("Se encontraron " + n + " registro(s). Toca 'Importar' para guardarlos.");
                ...
                requireActivity().runOnUiThread(() -> {
                    progressImportar.setVisibility(View.GONE);
                    Toast.makeText(requireContext(), "Error al leer el archivo: " + e.getMessage(),
